    ...
    public class AdminLoginService : IAdminLoginService
    {
        ...
        public async Task<LoginUserDTO> Login(LoginUserDTO user)
        {
            try
            {
                ...
                if (user == null)
                {
                    _logger.LogWarning("Invalid user login attempt with null user.");
                    throw new InvalidUserException();
                }
        ...
        private bool ComparePasswords(byte[] password, byte[] userPassword)
        {
            if (password == null || userPassword == null || password.Length != userPassword.Length)
            {
                return false;
            }
        ...
        public async Task<LoginUserDTO> Register(RegisterAdminDTO user)
        {
            try
            {
                ...
                if (user == null)
                {
                    _logger.LogWarning("Invalid registration attempt with null user.");
                    throw new InvalidUserException();
                }
